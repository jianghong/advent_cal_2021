...
fn part2() {
    ...
    let curr_path_string = String::new();
    let mut memoized_paths: HashMap<String, u64> = HashMap::new();
    find_min_path(&0, 0, 0, 0, curr_path, curr_path_string, &grid, visited_grid, end_i, end_j, None, &mut memoized_paths);
    // println!("memoized paths {:?}", memoized_paths);
    let mut min_path = 0;
    let mut min_path_string = String::new();
    for (k, v) in memoized_paths.iter() {
        if min_path == 0 || v < &min_path {
            min_path = *v;
            min_path_string = k.clone();
        }
    }
    let sum_chars = min_path_string.chars().map(|c| c.to_digit(10).unwrap() ).sum::<u32>();
    println!("part 2 {}", sum_chars);
}
...
fn find_min_path(
    ...
    curr_path_str: String,
    grid: &Vec<Vec<u32>>,
    visited_grid: Vec<Vec<bool>>,
    end_i: usize,
    end_j: usize,
    prev_min: Option<u64>,
    memoized_paths: &mut HashMap<String, u64>
) -> u64
{
    // println!("i {} j {}", i, j);
    let new_risk = curr_risk + *val as u64;
    let mut new_path = curr_path.clone();
    let new_path_string = format!("{}{}", curr_path_str.clone(), val);
    ...
    if i == end_i && j == end_j {
        ...
        memoized_paths.insert(new_path_string, new_risk);
        return new_risk;
    }
    ...
    for neighbor in neighbor_vals.clone() {
        if !visited_grid[neighbor.1][neighbor.2] {
            ...
            let neighbor_path_string = format!("{}{}", new_path_string.clone(), neighbor.0);
            let memoized_risk = memoized_paths.get(&neighbor_path_string);
            ...
            let path_risk = if let Some(memoized_risk) = memoized_risk {
                ...
                if *memoized_risk == u64::MAX {
                    *memoized_risk
                    ...
                    new_risk + *memoized_risk
                }
            } else {
                // println!("min_so_far {:?}", min_so_far);
                find_min_path(&grid[neighbor.1][neighbor.2], neighbor.1, neighbor.2,
                    new_risk, new_path.clone(), new_path_string.clone(), grid, visited_grid.clone(), end_i, end_j,
...
;
    ...
    for neighbor in neighbor_vals.clone() {
        let neighbor_path_string = format!("{}{}", new_path_string.clone(), neighbor.0);
        let t = memoized_paths.get(&neighbor_path_string).unwrap_or(&u64::MAX);
    ...
    let memoized_val = if min_memoized_neighbor_risk == u64::MAX {
        ...
        min_memoized_neighbor_risk  + new_risk
    };
    memoized_paths.insert(new_path_string,  memoized_val);
