...
fn part2() {
    ...
    let mut starting_point = (29, 100);
    // add (9, 0) .. (9, 9) to possible ends
    for i in 100..200 {
        possible_ends.push((199 , i));
        possible_ends.push((i, 199));
    }
    let mut min_endpoint: Option<(u64, usize, usize)> = None;
    for possible_end in possible_ends {
        let end_i = possible_end.0;
        let end_j = possible_end.1;
        let visited_grid = vec![vec![false; max_y + 1]; max_x + 1];
        let curr_path = Vec::new();
        let mut memoized_paths: HashMap<(usize, usize), u64> = HashMap::new();
        find_min_path(&0, starting_point.0, starting_point.1, 0, curr_path, &grid, visited_grid, end_i, end_j, None, &mut memoized_paths);
        // println!("memoized paths {:?}", memoized_paths);
        let result = min(memoized_paths.get(&(0, 1)).unwrap_or(&u64::MAX), memoized_paths.get(&(1, 0)).unwrap_or(&u64::MAX));
        if let Some(m) = min_endpoint {
            if result < &m.0 {
                min_endpoint = Some((*result, end_i, end_j));
            }
        } else {
            min_endpoint = Some((*result, end_i, end_j));
        }
    ...
    println!("min risk: {:?}", min_endpoint.unwrap());
}
    ...
    for neighbor in neighbor_vals.clone() {
        if !visited_grid[neighbor.1][neighbor.2] {
            ...
            let path_risk = if let Some(memoized_risk) = memoized_risk {
                ...
                find_min_path2(&grid[neighbor.1][neighbor.2], neighbor.1, neighbor.2,
